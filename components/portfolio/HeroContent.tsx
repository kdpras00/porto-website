      ...
      setDisplayText((oldText) =>
        oldText
          ...
          .map((char, index) => {
            ...
            return String.fromCharCode(65 + Math.floor(Math.random() * 26)); // Random uppercase letter A-Z
          })
          .join(""),
      );
...
export default function HeroContent() {
  ...
  return (
    <div className="relative z-20 flex-1 flex flex-col items-start justify-center pointer-events-none mt-20 px-4 md:px-10">
      ...
      {/* Typewriter Bio */}
      <div className="absolute bottom-32 right-8 md:bottom-36 md:right-24 max-w-sm md:max-w-md text-right">
        <motion.p
          key={bioKey}
          className="font-sans text-sm md:text-base text-white/70 leading-relaxed"
          initial="hidden"
          animate="visible"
          variants={{
            hidden: {},
